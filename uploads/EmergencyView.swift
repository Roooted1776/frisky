        ...
        NavigationView {
            ScrollView {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    // SIDE-BY-SIDE SCAN + CALL
                    HStack(spacing: 8) {
                        QuickActionCard(
                            icon: "wave.3.right.circle",
                            label: "Scan bracelet",
                            caption: "Tap band — browser opens their card.",
                            action: {}
                        )
                        QuickActionCard(
                            icon: "phone.fill",
                            label: "Call contacts",
                            caption: "iPhone asks before placing call.",
                            action: {}
                        )
                    }
                    .padding(.bottom, 4)
...
// MARK: - Quick Action Card (side-by-side)
struct QuickActionCard: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .overlay(RoundedRectangle(cornerRadius: 16).stroke(Color.redmedDivider, lineWidth: 1))
        }
        .buttonStyle(.plain)
    }
}
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            Button { withAnimation(.easeInOut(duration: 0.2)) { showSatellite.toggle() } } label: {
                HStack {
                    Image(systemName: "antenna.radiowaves.left.and.right").foregroundColor(.redmedAccent)
                    Text("No cell signal?").font(.system(size: 14, weight: .bold)).foregroundColor(.redmedAccent)
                    Spacer()
                    Image(systemName: showSatellite ? "chevron.up" : "chevron.down")
                        .font(.system(size: 12, weight: .semibold)).foregroundColor(.redmedMuted)
                }
            }
            .padding(14)
            ...
            if showSatellite {
                VStack(spacing: 8) {
                    Text("iPhone 14+ (iOS 16.1+): hold Side + Volume until Emergency SOS appears, or Settings → Emergency SOS.")
                        .font(.system(size: 11)).foregroundColor(.redmedMuted).lineSpacing(3)
