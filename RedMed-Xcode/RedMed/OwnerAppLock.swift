...
/// Owner-app Face ID gate. System prompt only — no lock page, heart, or Proceed.
/// Locked layer is cream-only so PHI cannot flash. Never wraps passerby
/// tapper / public NFC card.
///
...
struct OwnerAppLock<Content: View>: View {
    ...
    var body: some View {
        ZStack {
            ...
            if gate == .locked {
                // Cream only — no BrandLogo / heart / Proceed.
                Color.redmedBg
                    .ignoresSafeArea()
    ...
    private func handleScenePhase(_ phase: ScenePhase) {
        ...
        case .inactive:
            ...
            guard !TapCardPresentation.isVisible else { return }
            OwnerLockPresentation.holdSwitcherCover = true
        ...
        case .background:
            inactiveLockTask?.cancel()
            if TapCardPresentation.isVisible { return }
            OwnerLockPresentation.holdSwitcherCover = true
