...
9. `AppConfig.BraceletRF` owns tap-distance + iOS Background Tag Reading copy
   (no hardcoded inches). BTR can still open `tapper.html` (unlocked, screen
   on, ~1–2″); write does not change likelihood; do not claim “no background
   NFC” without the Apple OS caveat.
