...
- Console noise to ignore (Apple / Simulator, not RedMed bugs):
  ...
  - `com.apple.CoreMotion.plist` / `NSCocoaErrorDomain Code=257` (Managed Preferences — CMMotionManager still works)
  - `sandbox_extension_issue_file` / `Could not create a sandbox extension` for `RedMed.app`
  - `WebContent[…] Couldn't open <private>` (No such file / Permission denied)
