...
6. Owner lock: every launch is cream atmosphere + small original medical lock
   glyph (not BrandLogo, not Apple Face ID mark) under auto Face ID before Main
   (Unlock = cream dock after cancel / mismatch). Fresh install
...
- Cold launch: LaunchBackground only on UILaunchScreen (no system BrandLogo).
  Every owner launch: auto Face ID over cream atmosphere + small medical lock
  glyph before Main (no decorative BrandLogo; Unlock cream dock after cancel / mismatch).
