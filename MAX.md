...
- Owner open is Before you continue (every cold start), then Main after Agree. Same session stays in Main. No cream Face ID lock in front. Face ID is Edit / Save / Erase only. Crash motion runs while owner Main is in the foreground; CoreMotion stops in background. Armed siren can keep going.
...
After a pull: **Product → Clean Build Folder**, Stop, Run. Face ID test path: **Run Without Debugging**. Runtime Issue breakpoints live in xcuserdata (not git) — delete them if it still says Paused on iPhone. `Thread 1: signal SIGTERM` at `mach_msg2_trap` (Simulator) or `SIGKILL` at dyld `lldb_image_notifier` (iPhone) is Stop, not a crash; scheme `lldbinit` ignores both.
