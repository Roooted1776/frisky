...
|------|--------------------------|
| Live HTTPS `/tapper/` | `BASE=https://<host> ./scripts/smoke-pages.sh` returns OK. `AppConfig.medicalCardBaseURL` is that host. Interim `https://redmed-emergency.maxaguilaraasted.workers.dev/tapper/` is live. A 404 there means this gate is red ([`domain.md`](domain.md)). |
