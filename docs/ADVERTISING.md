...
|------|--------------------------|
| Live HTTPS `/tapper/` | `BASE=https://redmed.live ./scripts/smoke-pages.sh` returns OK. `AppConfig.medicalCardBaseURL` is `https://redmed.live/tapper/` (Hostinger static). A parking/404 host means this gate is red ([`domain.md`](domain.md)). |
