...
- **Do not** fork a second Privacy policy. Source of truth is in-app Help → Privacy (`RedMed-Xcode/RedMed/Help.html`). When Connect or the band host needs a Privacy Policy URL, serve that same Privacy text (tagged/hashed on the live host) — never a separate git tree or rewritten public page.
