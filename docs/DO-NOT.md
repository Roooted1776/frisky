...
- **Do not** sell blank bands, advertise a blank-band SKU, or tell people to program NTAG216 with Shortcuts / NFC Tools while CoreNFC is parked. `AppConfig.nfcHardwareEnabled` stays `false` (no NFC entitlement, no `NFCReaderUsageDescription`) until paid Program + NFC Tag Reading on `com.redmed.app` — see [`NFC-RESTORE.md`](NFC-RESTORE.md). Share Band URL / Preview are pack-only; they are not a write path and not a buy CTA. Factory “NDEF blank unlocked” is a chip procurement spec, not storefront copy.
