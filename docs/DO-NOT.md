...
- **Do not** sell blank bands, advertise a blank-band SKU, or tell people to program NTAG216 with Shortcuts / NFC Tools until NFC Tag Reading is live on a paid App ID and owner Write is proven on device — see [`NFC-RESTORE.md`](NFC-RESTORE.md). In-repo lockstep (`nfcHardwareEnabled`, TAG entitlement, `NFCReaderUsageDescription`) is restored; portal + Xcode capability are not. Share Band URL / Preview are not a buy CTA. Factory “NDEF blank unlocked” is a chip procurement spec, not storefront copy.
