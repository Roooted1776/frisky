...
- **Owner data independence:** `NFCWriter` / `ProfileNFCCodec` write only
  ...
  base is `https://redmed-emergency.maxaguilaraasted.workers.dev/tapper/`. Flip to a
  custom domain only after `docs/domain.md` cutover is green. No vendor tag-management
  cloud, no social/short-link redirect, no App Store URL on the chip, no BLE. Profile
  lives in `#d=` only; the Worker serves the shell.
