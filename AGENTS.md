...
watermark hangs with no sheet; `didAutoPromptThisLock` blocks re-prompt while
the Face ID sheet holds `.inactive`). Unlock is retry after cancel / mismatch.
Fresh install unlocks into empty tabs after
